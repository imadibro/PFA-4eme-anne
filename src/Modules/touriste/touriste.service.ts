...
export class TouristeService {
  ...
  async findAll(page: number, limit: number, search?: string): Promise<PaginatedResult<TouristeDto>> {
    try {
      ...
      return { data: touristeResponse, total, page, limit: validLimit, totalPages };
    } catch (error) {
